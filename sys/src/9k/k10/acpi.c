...
get64(uchar *p)
{
	return ((u64int)get32(p+4))|get32(p);
}
...
struct Mcfg
{
	u64int	base;	/* base of enhanced configuration mechanism */
	u16int	pciseg;	/* pci segment group number */
	u8int	start;		/* first pci bus number covered by this */
	u8int	end;		/* last pci bus number covered */
	/* 4 reserved */
};
static Mcfg mcfg[8];
static int	nmcfg;
