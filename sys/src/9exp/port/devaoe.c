...
struct Aoedev {
	...
	int	unit;
	int	lasttag;
	int	nframes;		/* BOTCH: need to track diff b/w maxframes & nframes */
	Frame	*frames;
	vlong	bsize;
	vlong	realbsize;
...
unitctlwrite(Aoedev *d, void *db, long n)
{
	...
	case Setsize:
		...
		if(cb->nf == 2){
			...
			if(bsize % Aoesectsz)
				cmderror(cb, "drive size must be sector aligned");
		}
		d->bsize = bsize;
		break;
	default:
...
netbind(char *path)
{
	...
	snprint(addr, sizeof addr, "%s!0x%x", path, ETAOE);
